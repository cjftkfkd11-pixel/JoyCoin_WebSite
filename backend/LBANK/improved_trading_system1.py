...
class TradingChart:
    ...
    def plot_growth_chart(self, save_path: str = None):
        ...
        ax1.set_title('🚀 자연스러운 성장 + 박스권 거래', fontsize=16, fontweight='bold')
        ...
        if save_path:
            ...
            print(f"📊 자연스러운 성장 차트 저장됨: {save_path}")
...
class SmartGrowthTradingSystem:
    """🔥 스마트 성장 거래 시스템 - 자연스러운 박스권 + 점진적 상승 + 스마트 주문 관리"""
    ...
    def __init__(self, api_key, api_secret):
        ...
        logger.info("자연스러운 성장 거래 시스템 초기화 완료")
    ...
    def generate_smart_trade_amount(self, size_type: str, current_price: float, balance: Dict[str, float], balance_status: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            if mode == 'emergency_buy':
                ...
                # 🔥 개선된 성장 모드 - 자연스러운 박스권 + 점진적 상승
                if urgency == 'high':
                    # 높은 긴급도 - 매수 우세하지만 매도도 활발
                    buy_ratio = 0.65  # 65% (기존 90% → 65%)
                    sell_ratio = 0.35  # 35% (기존 10% → 35%)
                    base_amount *= 1.3
                elif urgency == 'medium':
                    # 중간 긴급도 - 균형잡힌 박스권
                    buy_ratio = 0.60  # 60% (기존 80% → 60%)
                    sell_ratio = 0.40  # 40% (기존 20% → 40%)
                    base_amount *= 1.1
                else:
                    # 낮은 긴급도 - 거의 균형
                    buy_ratio = 0.55  # 55% (기존 70% → 55%)
                    sell_ratio = 0.45  # 45% (기존 30% → 45%)
                
                print(f"      - 📈 자연스러운 성장: 매수 {buy_ratio*100:.0f}% : 매도 {sell_ratio*100:.0f}% (박스권+상승)")
                    
            else:  # balance 모드
                # 균형 모드 - 완전한 균형
                buy_ratio = random.uniform(0.45, 0.55)  # 45-55%
    ...
    def calculate_smart_price(self, trade_type: str, current_price: float, mode: str, urgency: str) -> float:
        ...
        try:
            if mode == 'emergency_buy':
                ...
                # 🔥 개선된 성장 모드 - 자연스러운 박스권 + 점진적 상승 압력
                if trade_type == 'buy':
                    # 매수는 현재가보다 살짝 높게 (상승 압력)
                    if urgency == 'high':
                    ...
                    # 🔥 매도는 현재가보다 살짝 낮게 (자연스러운 하락 허용)
                    if urgency == 'high':
                        spread = random.uniform(0.0002, 0.001)  # 0.02-0.1% 낮게
                    else:
                        spread = random.uniform(0.0005, 0.0015)  # 0.05-0.15% 낮게
                    
                    target_price = current_price * (1 - spread)
                    print(f"         🔥 성장모드 매도: 현재가 -{spread*100:.2f}% (자연스러운 하락)")
    ...
    def execute_enhanced_smart_growth_trade(self, current_price: float, balance: Dict[str, float]) -> Dict[str, Any]:
        """🚀 개선된 자연스러운 성장 거래 - 자연스러운 박스권 + 점진적 상승"""
        try:
            print(f"   🚀 자연스러운 성장 거래 실행:")
            ...
            trade_params = self.generate_smart_trade_amount(size_type, current_price, balance, balance_status)
            ...
            if (feasibility['can_buy'] and 
                trade_params['buy_amount'] >= self.min_order_size and
                balance['usdt'] >= trade_params['buy_amount'] * current_price):
                
                print(f"      🛒 매수 실행: {trade_params['buy_amount']:,.0f} SPSI")
                
                buy_result = self.execute_smart_buy_with_retry(
                    current_price, balance, self.current_mode, 
                    balance_status['urgency'], max_retries=2
                )
                ...
                if buy_result['success']:
                    ...
                    # 🔥 거래량 통계 업데이트
                    self.total_volume_today += buy_result['amount']
            ...
            # 5. 🔥 개선된 매도 실행 (확률 제거, 비율 기반)
            if (feasibility['can_sell'] and 
                trade_params['sell_amount'] >= self.min_order_size and
                balance['spsi'] >= trade_params['sell_amount']):
                
                # 🔥 Growth 모드에서도 매도 적극 실행
                should_sell = True
                
                # 긴급 모드별 제외 조건
                if self.current_mode == 'emergency_buy':
                    should_sell = False  # 긴급 매수 모드에서만 매도 금지
                    print(f"      ⏸️ 긴급 매수 모드 - 매도 스킵")
                
                if should_sell:
                    time.sleep(random.uniform(0.5, 2.0))  # 매수 후 잠시 대기
                    
                    print(f"      💰 매도 실행: {trade_params['sell_amount']:,.0f} SPSI")
                    ...
                    print(f"         - 현재가 대비: {((sell_price/current_price-1)*100):+.2f}%")
                    
                    sell_order_id = self.enhanced_place_order('sell', trade_params['sell_amount'], sell_price)
                    ...
                    if sell_order_id:
                        ...
                        # 🔥 거래량 통계 업데이트
                        self.total_volume_today += trade_params['sell_amount']
                        
                        self.chart.add_trade_data('sell', trade_params['sell_amount'], sell_price, True, size_type)
                        print(f"      ✅ 매도 성공!")
                    else:
                        print(f"      ❌ 매도 실패")
                        self.chart.add_trade_data('sell', trade_params['sell_amount'], sell_price, False, size_type)
            
            results['executed_trades'] = executed_trades
            
            # 6. 결과 요약
            if executed_trades > 0:
                print(f"   📊 자연스러운 성장 거래 결과:")
                print(f"      - 실행거래: {executed_trades}회")
                print(f"      - 매수: {'✅' if results['buy_success'] else '❌'}")
                print(f"      - 매도: {'✅' if results['sell_success'] else '❌'}")
                print(f"      - 현재 모드: {self.current_mode}")
                print(f"      - 비율: 매수{trade_params['buy_ratio']*100:.0f}% : 매도{trade_params['sell_ratio']*100:.0f}%")
            ...
            print(f"   💥 자연스러운 성장 거래 오류: {e}")
            logger.error(f"자연스러운 성장 거래 오류: {e}")
            return {'executed_trades': 0, 'error': str(e)}

    def get_daily_volume_from_chart(self) -> float:
        """📊 차트 데이터에서 오늘 거래량 계산"""
        try:
            ...
            # 성공한 거래만 카운트
            for trade in self.chart.trade_history:
                if (trade['success'] and 
                    trade['time'].date() == today):
                    daily_volume += trade['amount']
            
            return daily_volume
        except Exception as e:
            print(f"   ⚠️ 일일 거래량 계산 오류: {e}")
            return self.total_volume_today
        """🔥 동적 거래 간격 (모드별)"""
    ...
    def execute_smart_growth_cycle(self) -> bool:
        """🔥 자연스러운 성장 자가매매 사이클 (개선된 버전)"""
        try:
            print("   🚀 자연스러운 성장 자가매매 사이클 시작...")
            ...
            if results['executed_trades'] > 0:
                print(f"   ✅ 자연스러운 성장 거래 성공 ({results['executed_trades']} 거래)")
                return True
            else:
                print(f"   ❌ 모든 거래 실패")
                return False
            ...
            print(f"   💥 자연스러운 성장 거래 사이클 오류: {e}")
            logger.error(f"자연스러운 성장 거래 사이클 오류: {e}")
            return False
    ...
    def diagnose_buy_issues(self):
        ...
        if balance['usdt'] >= 5:
            print("   2. 1회 자연스러운 성장 테스트 (메뉴 3번)")
        
        if feasibility['can_buy']:
            print("   3. 실시간 모니터링 시작 (메뉴 8번)")
        
        print("   4. 자연스러운 성장 차트 확인 (메뉴 7번)")
    ...
    def create_buy_success_report(self):
        ...
        print("🚀 자연스러운 성장 테스트 실행...")
        
        # 거래 전 상태
        before_balance = self.get_account_balance()
        current_price = self.get_reference_price()
        before_volume = self.get_daily_volume_from_chart()  # 거래 전 누적 거래량
        ...
        if before_balance and current_price:
            ...
            print(f"   - 오늘 누적 거래량: {before_volume:,.0f} SPSI")
        ...
        if result:
            ...
            if after_balance and before_balance:
                ...
                if volume_diff > 0:
                    print(f"   - 결과: 🟢 거래량 {volume_diff:,.0f} SPSI 증가 성공!")
                else:
                    print(f"   - 결과: ⚠️ 체결된 거래량 없음 (주문 대기중)")
            
            print("\n💡 다음 단계:")
            print("   - 메뉴 11번: 상세 주문 현황 확인")
            print("   - 메뉴 7번: 자연스러운 성장 차트 확인")
            print("   - 메뉴 4번: 자연스러운 자가매매 시작")
            return True
        else:
            print("\n❌ 자연스러운 성장 테스트 실패!")
    ...
    def start_self_trading(self):
        ...
        self.running = True
        print("🚀 자연스러운 성장 자가매매 시스템 시작!")
        print(f"🎯 특징: 매수/매도 균형잡힌 박스권 + 점진적 상승")
        print(f"📈 Growth 모드: 매수 55-65% + 매도 35-45% (자연스러운 박스권)")
        print(f"⚖️ Balance 모드: 매수 45-55% + 매도 45-55% (완전 균형)")
        ...
        def trading_loop():
            ...
            while self.running:
                try:
                    ...
                    print(f"\n⏰ {datetime.now().strftime('%H:%M:%S')} - 자연스러운 성장 자가매매 실행")
                    ...
                    if success:
                        ...
                        # 🔥 개선된 실시간 통계 출력
                        chart_volume = self.get_daily_volume_from_chart()  # 차트에서 실제 거래량
                        print(f"   📈 실시간 통계:")
                        print(f"      - 오늘 거래량: {chart_volume:,.0f} SPSI (실제 체결)")
                        print(f"      - 주문 거래량: {self.total_volume_today:,.0f} SPSI (주문 기준)")
                        ...
                        if self.successful_buys > 0 or self.successful_sells > 0:
                            total_success = self.successful_buys + self.successful_sells
                            buy_pct = (self.successful_buys / total_success) * 100
                            sell_pct = (self.successful_sells / total_success) * 100
                            print(f"      - 매수/매도 비율: {buy_pct:.0f}% : {sell_pct:.0f}% {'✅균형' if 40 <= buy_pct <= 60 else '⚠️불균형'}")
                        ...
                        if self.current_mode == 'growth':
                            ...
                            print(f"      - 📈 Growth: 자연스러운 박스권 + 점진적 상승")
                        elif self.current_mode == 'balance':
                            print(f"      - ⚖️ Balance: 완전 균형 박스권 거래")
                        print(f"      - 강제 매수: {self.mode_stats['forced_buys']}회")
    ...
    def get_status(self):
        ...
        try:
            ...
            if self.current_mode == 'growth':
                print(f"   📈 Growth 모드: 매수 우세 (55-65%) + 자연스러운 매도 (35-45%)")
            elif self.current_mode == 'balance':
                print(f"   ⚖️ Balance 모드: 완전 균형 (45-55% : 45-55%)")
            elif self.current_mode == 'emergency_buy':
                print(f"   🚨 긴급 매수 모드: 매수 100% (SPSI 복구 중)")
            elif self.current_mode == 'emergency_sell':
                print(f"   🚨 긴급 매도 모드: 매도 100% (USDT 확보 중)")
            ...
            print(f"📊 거래 통계:")
            print(f"   - 오늘 총 거래량: {chart_volume:,.0f} SPSI (실제 체결)")
            print(f"   - 주문 총 거래량: {self.total_volume_today:,.0f} SPSI (주문 기준)")
            ...
            if self.successful_buys > 0 or self.successful_sells > 0:
                total_success = self.successful_buys + self.successful_sells
                buy_ratio = (self.successful_buys / total_success) * 100
                sell_ratio = (self.successful_sells / total_success) * 100
                print(f"   - 매수/매도 균형: {buy_ratio:.0f}% : {sell_ratio:.0f}%")
                
                if 40 <= buy_ratio <= 60:
                    print(f"   - 균형 평가: ✅ 건전한 박스권 거래")
                elif buy_ratio > 70:
                    print(f"   - 균형 평가: 📈 매수 우세 (상승 압력)")
                elif sell_ratio > 70:
                    print(f"   - 균형 평가: 📉 매도 우세 (하락 압력)")
                else:
                    print(f"   - 균형 평가: 🔵 적절한 방향성")
    ...
    def show_growth_chart(self):
        """자연스러운 성장 차트 표시"""
        try:
            print("📊 자연스러운 성장 차트 생성 중...")
            
            # 차트 생성
            chart_filename = f"natural_growth_chart_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            ...
            if 'balance_analysis' in stats:
                ...
                if 40 <= balance['buy_ratio'] <= 60:
                    print(f"   - 평가: ✅ 건전한 박스권 (자연스러운 등락)")
                elif balance['buy_ratio'] > 65:
                    print(f"   - 평가: 📈 매수 우위 (상승 압력 강함)")
                elif balance['sell_ratio'] > 65:
                    print(f"   - 평가: 📉 매도 우위 (하락 압력 강함)")
                else:
                    print(f"   - 평가: 🔵 적절한 방향성 (점진적 성장)")
            
        except Exception as e:
            print(f"❌ 자연스러운 성장 차트 생성 오류: {e}")
            logger.error(f"자연스러운 성장 차트 생성 오류: {e}")

def main():
    print("🚀 자연스러운 성장 LBank 자가매매 시스템")
    print("🎯 특징: 매수/매도 균형잡힌 박스권 + 점진적 상승")
    print("💡 개선사항: Growth 모드에서도 35-45% 매도로 자연스러운 등락")
    
    # matplotlib 설정
    try:
        import matplotlib
        matplotlib.use('Agg')
        print("📊 자연스러운 성장 차트 기능 활성화됨")
    except ImportError:
    ...
    try:
        ...
        while True:
            try:
                ...
                print("="*80)
                print("🎯 특징: 매수/매도 균형잡힌 박스권 + 점진적 상승")
                print("💡 개선: Growth 모드에서도 35-45% 매도로 자연스러운 등락")
                print("🔄 모드: 자연성장(55:45-65:35) ↔ 완전균형(50:50) 자동 전환")
                print("="*80)
                print("1. 💰 상태 확인 (모드 + 균형 + 주문 관리)")
                print("2. 🧪 시스템 테스트 (API + 모드 분석)")
                print("3. 🚀 자연스러운 성장 테스트 (매수/매도 균형)")
                print("4. 🔥 자연스러운 자가매매 시작")
                print("5. ⏹️ 자가매매 중지")
                print("6. 🧹 스마트 주문 정리 (매수5개+매도5개 유지)")
                print("7. 📊 자연스러운 성장 차트 보기")
                ...
                if choice == '1':
                    ...
                    print("\n⚠️ 실제 자연스러운 성장 거래가 실행됩니다!")
                    print("🚀 자연스러운 성장 테스트:")
                    print("   - 매수/매도 균형잡힌 거래 (Growth 모드에서도 35-45% 매도)")
                    print("   - 자연스러운 박스권 형성 + 점진적 상승")
                    ...
                    print("\n⚠️ 자연스러운 성장 자가매매 시작 주의사항:")
                    print("- Growth 모드: 매수 55-65% + 매도 35-45% (자연스러운 박스권)")
                    print("- Balance 모드: 매수 45-55% + 매도 45-55% (완전 균형)")
                    print("- 매수 실패시 자동 재시도 (최대 2회)")
                    print("- 주문 수를 매수5개+매도5개로 자동 관리")
                    print("- SPSI 잔고 상태를 지속적으로 모니터링")
                    print("- 자연스러운 가격 등락으로 건전한 시장 형성")
                    
                    confirm = input("\n정말 시작하시겠습니까? (y/N): ").strip().lower()
                    if confirm == 'y':
                        st.start_self_trading()
                        if st.running:
                            print("✅ 자연스러운 성장 자가매매 시스템이 시작되었습니다!")
                            print("💡 매수와 매도가 균형잡히게 실행되어 건전한 박스권을 형성합니다.")
                        else:
                            print("❌ 자가매매 시작 실패")
                    else:
                        print("자가매매 시작 취소됨")
                    ...
                    print("👋 자연스러운 성장 자가매매 시스템을 종료합니다.")
                    break
                    
                else:
                    print("❌ 잘못된 선택입니다. 0-11 사이의 번호를 입력하세요.")

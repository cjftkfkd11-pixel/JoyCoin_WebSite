...
export default function MyPage() {
  ...
  // 상태(Status)에 따른 색상 및 텍스트 변환 함수
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending': return <span className="px-2 py-1 bg-yellow-500/20 text-yellow-500 text-[10px] font-bold rounded-md">PENDING</span>;
      case 'approved': return <span className="px-2 py-1 bg-green-500/20 text-green-500 text-[10px] font-bold rounded-md">APPROVED</span>;
      case 'rejected': return <span className="px-2 py-1 bg-red-500/20 text-red-500 text-[10px] font-bold rounded-md">REJECTED</span>;
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] text-slate-600 uppercase border-b border-slate-800">
                  ...
                  <th className="pb-4 font-black">Network</th>
